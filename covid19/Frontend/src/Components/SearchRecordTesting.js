...
import { ToastContainer, toast } from 'react-toastify';
 ...
 class SearchRecordTesting extends React.Component{
    constructor(){
        super();
        this.state={
            adhaarNumber:'',
    ...
    async fetchData(){
        ...
        const response=await axios.get(`http://localhost:5040/testing/adhaar/${id}`)
          if(response.data==='')
          {
    ...
    render(){
        return(
            ...
            <div className="row ">
              ...
              <center><h3>Check Testing Appointment Status</h3></center>
              <br/><br/>
          ...
          {this.state.flag?
          <div><center><h3>Testing Detail</h3><br/>
          <div class="box"style={this.state.data && new Date(this.state.data.dateOfTesting) < new Date(new Date().setHours(0,0,0,0)) ? {borderColor: 'red'} : new Date(this.state.data.dateOfTesting).setHours(0,0,0,0) === new Date().setHours(0,0,0,0) ? {borderColor: 'green'} : {}}>
          <div><h4>Hi, {this.state.data.firstName} {this.state.data.lastName},</h4></div>
          <div style={this.state.data && new Date(this.state.data.dateOfTesting) < new Date(new Date().setHours(0,0,0,0)) ? {color: 'red'} : new Date(this.state.data.dateOfTesting).setHours(0,0,0,0) === new Date().setHours(0,0,0,0) ? {color: 'green'} : {color:'navy'}}><h7>Testing Appointment has been {this.state.data.status}</h7></div>
          </div>
          </center>
          <br></br>
  ...
  <tbody>
    <tr>
                            ...
                            <td>{this.state.data.dateOfTesting}</td>
                            <td>{this.state.data.timeOfTesting}</td>
...
export default SearchRecordTesting;